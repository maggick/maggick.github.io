...
meta:First April SSTIC challenge

<img class="align-left" src="/media/2015.04/sstic_init_res_512.PNG"
alt="initial pixel view" width="342">
...
In France we got a security event call SSTIC for which the tickets are very rare
therefore the SSTIC challenge allow the ones with the flag to reserve a ticket.
The challenge was post yesterday
<!-- PELICAN_END_SUMMARY -->

The first of April is always the occasion for some great pranks:
  * CERN confirm the existence of the Force:

twitter.com_CERN_status_583147747710263296.png

  * Google do some mirroring trick:

com.google.png

  * gentoo.org adopted a beautiful design:

gentoo.org.png

But one of them was released by the SSTIC an French security event which give
every year a difficult challenge than grant automatically a place to the event
(tickets are sold out in approximatly one minute)

We download the challenge :

    wget http://static.sstic.org/challenge2015/chlg-2015

Let's try something like string to see the result
...
Yeah that a message encrypt with open ssl and salt.
...
We launch the `dff hex viewer` and lock at it as a pixel image:

sstic_init_res_512.png

With the default resolution of 512 it does not lock very pretty, let's variate
...
We just need to open edit, turn it upside down and mirroring it to get the final
result:
