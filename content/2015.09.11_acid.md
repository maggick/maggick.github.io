...
category:security
tags:security, vulnhub
meta:security, vulnhub

<img class="align-left"
src="/media/2015.09/bg.jpg" alt="acid" width="262">

Since [Fart knocker](http://matthieukeller.com/2015/06/vulnhub-fart-knocker.html)
in June I have work on an other vulnhub machine:
[darknet](https://www.vulnhub.com/entry/darknet-10,120/). But this one is really
hard and get me stuck. I was a bit demotivated to continue vulnhub machines but
...
things came out:
  ...
  * Some hexa at the end of the page: 0x643239334c6d70775a773d3d
  * the title of the page (we can see it in the second nmap scan) is /Challenge
    and there is an other page at this address.
...
We got to the url (the /images/ is given with the background image of the first
web page):
...
with the small dictionnary (`/usr/share/dirbuster/directory-list-2.3-small.txt):
...
let us think that we can us command exploitation. If we add a `;` in the filed
and a command it seems to works, with `; ls` we got (I use
[burp](http://releases.portswigger.net/) for all my pentest, the output of the
command is in the reponse page):
...
(`bash_history`). In the `/home/acid/` there is something interessting but
empty:
...
[Fart knocker](http://matthieukeller.com/2015/06/vulnhub-fart-knocker.html))
without success:
