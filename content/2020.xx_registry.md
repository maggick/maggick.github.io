...
We found a backup archive on the system `/var/backup/blot.tgz`. We scp it on our
...
So we look at the file in .gitignore.

### app/config

The file `config.yml` tell us that there is sqlite database named bolt. The
other files does not contain usefull information
...
We try to brutforce the password using john. The hash algorithm is bcrypt and
the password doesn't seems to be on the top 10 000 (+ the three passwords found
earlier).

We are working on the backuped database. Let us just get the "production" one.
