...
The hostpage announce that the site was hacked.
...
![backup homepage](/media/2020.xx/forwardslash_02.png)
...
![dev access denied](/media/2020.xx/forwardslash_03.png)
...
Wit a simple combination of `grep` and `cut` we can extract the current filename
used by the program:
...
We can then create a temporary folder and try to get the file `user.txt` in pain
homedir by creating a symlink with the name of the file used by the backup
